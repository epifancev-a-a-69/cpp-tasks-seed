...
const std::vector<std::string> short_cases =
{
    "",
    "1",
    "12",
...
{
    for (const auto& s : short_cases)
    {
        auto original = cstr2v(s.c_str());

        auto encoded = base85::encode(original);
        auto decoded = base85::decode(encoded);

        EXPECT_EQ(decoded, original)
                << "Failed encoding/decoding: '" << s << "'";
    }
}
...
{
    std::vector<size_t> test_sizes = {1,  2,   3,   4,   5,    10,
                                      50, 100, 256, 500, 1000, 10000
                                     };
    ...
    for (size_t size : test_sizes)
    {
        ...
        EXPECT_EQ(original, decoded) << "Round-trip failed for size: " << size;
    }
}

TEST(Base85SpecialCases, ZeroBlockRoundTrip)
{
...
TEST(Base85SpecialCases, MultipleZeroBlocksRoundTrip)
{
...
TEST(Base85EdgeCases, AlternatingPatterns)
{
    for (size_t len = 1; len <= 16; ++len)
    {
        ...
        EXPECT_EQ(decoded, pattern)
                << "Failed for alternating pattern length: " << len;
    }
}
...
TEST(Base85StressTest, LargeData)
{
            ...
            })
    {
        ...
        std::cout << "Size: " << size << " bytes, time: " << duration.count()
                  << "ms" << std::endl;
    }
}
...
TEST(Base85Performance, EncodeDecodeSpeed)
{
    ...
    for (int i = 0; i < 10; ++i)
    {
        ...
        EXPECT_EQ(decoded, test_data)
                << "Performance test failed at iteration: " << i;
    }
}
